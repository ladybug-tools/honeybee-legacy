...
def getFloorCrvs(buildingMass, floorHeights, maxHeights):
    ...
    for h in floorHeights:
        ...
        #If the crvList cointains multiple curves, try curve booleaning them together to just get one curve.
        if len(crvList) > 1:
            try:
                newCurve = rc.Geometry.Curve.CreateBooleanUnion(crvList)
            except: pass
    ...
    return contourCrvs, splitters, crvAdjust, topInc, nurbsList
...
def splitFloorHeights(bldgMasses, bldgsFlr2FlrHeights, lb_preparation, lb_visualization):
    if len(bldgMasses)!=0:
        ...
        for bldgCount, mass in enumerate(initialMasses):
            ...
            if floorHeights!=[0]:
                floorCrvs, splitterSrfs, crvAdjust, topInc, nurbsList = getFloorCrvs(mass, floorHeights, maxHeights)
...
def cleanCurve(curve, curvePts):
    ...
    for pointCount, point in enumerate(curvePts):
        ...
        if testLine.DistanceTo(curvePts[pointCount-1], True) > tolerance and len(newPts) == 0:
            ...
            newPts.append(curvePts[pointCount-1])
        elif curveBool[pointCount-2] == True:
            newPts.append(curvePts[pointCount-1])
...
def splitPerimZones(mass, zoneDepth, floorCrvs, topInc, nurbsList):
    finalZones = []
    ...
    for curveCount, curve in enumerate(floorCrv):
        ...
        #If the geomtry fails the segment count test above but was originally a single nurbs curve, it is still possible to generate zones for it.
        ...
        if checkCurveSeg == True:
            ...
            for zone in floorZones:
                if zone == None:
                    noneTest.append(1)
                    print "The script has failed to create one of the zones because of a tolerance issue.  Try changing your Rhino Model's tolerance (prorbably ty increasing) and recompute the GH definition to generate zones correctly."
                else: pass
    ...
    if sc.sticky.has_key('ladybug_release')and sc.sticky.has_key('honeybee_release'):
        ...
        if floorHeights != []:
            ...
            for item in mass:
                ...
                contCrvs, splitters, flrCrvs, topIncl, nurbL = getFloorCrvs(item, [0, maxHeights-minHeights], maxHeights)
