...
For more information about Earthtube please see page 465 of the EnergyPlus input output reference at:
...
Provided by Honeybee 0.0.56
    ...
    Args:
        ...
        fan_n: This is the total fan efficiency (a decimal number between 0.0 and 1.0) the default is 1. This is a function of the fan and plays a role in determining the amount of energy consumed by the fan.        pipe_radius: This is the radius of the earth tube/pipe (in meters) the default is 0.5 meter. This plays a role in determining theamount of heat transferred from the surrounding soil to the air passing along the pipe. If the pipe has non-circular cross section, user can use the concept of hydraulic diameter where Radius = 2*Area/Perimeter.        : This is the thickness of the pipe wall (in meters) the default is 0.2 meters. This plays a role in determining the amountof heat transferred from the surrounding soil to the air passing along the pipe.
        pipe_length: This is the total length of the pipe (in meters) the default is 15 meters. This plays a role in determining the amount of heat transferred from the surrounding soil to the air passing along the pipe. As the length of the pipe becomes longer, the amount of the heat transfer becomes larger        pipe_depth: This is the depth of the pipe under the ground surface (in meters) the default is 3 meters. This plays a role in determining the temperature of the soil surrounding the pipe.
...
ghenv.Component.NickName = 'AddEarthtube'
ghenv.Component.Message = 'VER 0.0.56\nMar_19_2015'
ghenv.Component.Category = "Honeybee"
ghenv.Component.SubCategory = "12 | WIP" #"08 | Energy | Set Zone Properties"
#compatibleHBVersion = VER 0.0.56\nFEB_01_2015
#compatibleLBVersion = VER 0.0.59\nFEB_01_2015
try: ghenv.Component.AdditionalHelpFromDocStrings = "3" #"1"
except: pass
...
def checktheinputs(Schedules_,design_flowrate,mincooling_temp,maxheating_temp,delta_temp,earthtube_type,fan_Prise,fan_n,pipe_radius,pipe_depth,soil_condition,condition_groundsurface,pipe_thermal_conductivity):
    ...
    if (pipe_depth*3)+pipe_thickness <= pipe_radius:
   ...
   # Check soil condition input
    ...
    if not _epwFile and not _epwFile.endswith('.epw') and not _epwFile != 'C:\Example.epw':
        ...
        # Make sure condition_groundsurface and soil_condition are ints
    
    try: 
        if (int(condition_groundsurface)) or (int(soil_condition)): pass
        
    except:
        warnMsg= "condition_groundsurface and soil_condition must be integers!"
        print warnMsg
        w = gh.GH_RuntimeMessageLevel.Warning
        ghenv.Component.AddRuntimeMessage(w, warnMsg)
        return -1 # Note to self just print a runtime warning DOES NOT stop the component from executing here I am making it return -1 so that main will stop executing due to ... on line ... XXX this is what Mostapha has done and its good practice.
        
        # Make sure earthtube_type is an int
        
    try: 
        if int(earthtube_type): pass
        
    except:
        warnMsg= "earthtube_type must be an integer! "
        print warnMsg
        w = gh.GH_RuntimeMessageLevel.Warning
        ghenv.Component.AddRuntimeMessage(w, warnMsg)
        return -1 # Note to self just print a runtime warning DOES NOT stop the component from executing here I am making it return -1 so that main will stop executing due to ... on line ... XXX this is what Mostapha has done and its good practice.
...
def main(_HBZones,Schedules_,design_flowrate,mincooling_temp,maxheating_temp,delta_temp,earthtube_type,fan_Prise,fan_n,pipe_radius,pipe_thickness,pipe_length,pipe_depth,soil_condition,condition_groundsurface,pipe_thermal_conductivity):
...
if checktheinputs(Schedules_,design_flowrate,mincooling_temp,maxheating_temp,delta_temp,earthtube_type,fan_Prise,fan_n,pipe_radius,pipe_depth,soil_condition,condition_groundsurface,pipe_thermal_conductivity) != -1:

    if _HBZones and _HBZones[0]!= None:
        earthtube_HBZones = main(_HBZones,Schedules_,design_flowrate,mincooling_temp,maxheating_temp,delta_temp,earthtube_type,fan_Prise,fan_n,pipe_radius,pipe_thickness,pipe_length,pipe_depth,soil_condition,condition_groundsurface,pipe_thermal_conductivity)