...
Provided by Honeybee 0.0.56
    ...
    Args:
        ...
        simple_or_analytical: An integer of 1 or 2 that defines whether the wind turbine is simple or analytical,
        ...
        max_power_coefficient: Used only with the simple model, this is the maximum fraction of power extraction possible from the ambient wind. This value can be calculated from the power curve published in most manufacturers' specifications by using the kinetic energy equation as
...
def checktheinputs(name_,simple_or_analytical,power_coefficients,powercontrol,rotor_speed,rotor_diameter,overall_height,power_output,rated_wind_speed,cut_in_windspeed,cut_out_windspeed,overall_turbine_n,max_tip_speed_ratio,max_power_coefficient,local_av_windspeed,height_local_metrological_station,turbine_cost):
    ...
    if (simple_or_analytical != 1) and (simple_or_analytical != 2):
        
        warn = "simple_or_analytical must be the integer 1 or 2 \n"+\
    ...
    if max_power_coefficient == None:
        print "The field max_power_coefficient must be specified!"
        w = gh.GH_RuntimeMessageLevel.Warning
        ghenv.Component.AddRuntimeMessage(w, "The field max_power_coefficient must be specified!")
        return -1
...
def main(name_,simple_or_analytical,powercontrol,rotor_speed,rotor_diameter,overall_height,number_of_blades,power_output,rated_wind_speed,cut_in_windspeed,cut_out_windspeed,overall_turbine_n,max_tip_speed_ratio,max_power_coefficient,local_av_windspeed,height_local_metrological_station,turbine_cost,power_coefficients):
    ...
    print " The cost for the turbine is " + str(turbine_cost) + " British Pounds"
        
        
    if simple_or_analytical == 1:
        
        print " This turbine is using the simple model and the Maximum Power Coefficient for the turbine is "+ str(max_power_coefficient)
...
def checktheinputs_template(template_medium_turbine,template_large_turbin,simple_or_analytical):
...
if checktheinputs_template(template_medium_turbine,template_large_turbine,simple_or_analytical) != -1:
    
    if template_medium_turbine == True:
        
        if checknametemplate(name_) != -1:
            
            if simple_or_analytical == None:
                simple_or_analytical = 1
...
if checktheinputs(name_,simple_or_analytical,power_coefficients,powercontrol,rotor_speed,rotor_diameter,overall_height,power_output,rated_wind_speed,cut_in_windspeed,cut_out_windspeed,overall_turbine_n,max_tip_speed_ratio,max_power_coefficient,local_av_windspeed,height_local_metrological_station,turbine_cost) != -1:
    
    HB_windturbine = main(name_,simple_or_analytical,powercontrol,rotor_speed,rotor_diameter,overall_height,number_of_blades,power_output,rated_wind_speed,cut_in_windspeed,cut_out_windspeed,overall_turbine_n,max_tip_speed_ratio,max_power_coefficient,local_av_windspeed,height_local_metrological_station,turbine_cost,power_coefficients)
