...
"""
Use this component to color zones based on EP result data out of the Read EP Result component or zone comfort analyses out of the comfort calculator components.
_
...
Provided by Ladybug 0.0.57
    ...
    Args:
        ...
        _HBZones: The HBZones out of any of the HB components that generate or alter zones.  Note that these should ideally be the zones that are fed into the Run Energy Simulation component or zones read back into Grasshopper from the Import idf component in order to ensure alignment with the EP Result data.
        ===============: ...
...
ghenv.Component.Name = "Ladybug_Color Zones by EP Result"
...
3: ["zoneNumbers", "A set of surfaces indicating the number of each zone as it corresponds to the branches in the result file."],
...
def checkTheInputs():
    ...
    if checkData2 == True:
        ...
        if "Floor Normalized" in dataType:
            normable = False
            total = True
        elif "Total Thermal Energy for Zone" in dataType: normable = True
        elif "Total Energy for Zone" in dataType: normable = True
        elif "Heating - Cooling Energy for Zone" in dataType: normable = True
        elif "Cooling Energy for Zone" in dataType: normable = True
        elif "Heating Energy for Zone" in dataType: normable = True
        elif "Electric Lighting Energy for Zone" in dataType: normable = True
        elif "Electric Equipment Energy for Zone" in dataType: normable = True
        elif "People Energy for Zone" in dataType: normable = True
        elif "Total Solar Gain for Zone" in dataType: normable = True
        elif "Solar Beam Energy for Zone" in dataType: normable = True
        elif "Solar Diffuse Energy for Zone" in dataType: normable = True
        elif "Infiltration Energy Loss/Gain for Zone" in dataType: normable = True
        elif "Operative Temperature for Zone" in dataType: normable = False
        elif "Air Temperature for Zone" in dataType: normable = False
        elif "Radiant Temperature for Zone" in dataType: normable = False
...
def main(zoneValues, zones, zoneFloors, title, legendTitle, lb_preparation, lb_visualization, legendPar):
    ...
    zoneNumInt = range(len(zoneCentPts))
    zoneNumStr = []
    for int in zoneNumInt:
        zoneNumStr.append(str(int))
    zoneNum = text2srf(zoneNumStr, zoneCentPts, legendFont, textSize)
    zoneNumFinal = []
    for list in zoneNum:
        for brep in list:
...
if _runIt == True and checkData == True and _HBZones != [] and finalCheck == True and zoneValues != []:
    zoneColors, zoneBreps, zoneColoredMesh, zoneWireFrame, legendInit, legendBasePt, zoneNumbers = main(zoneValues, _HBZones, zoneFloors, title, legendTitle, lb_preparation, lb_visualization, legendPar_)
    #Unpack the legend.
    legend = []
