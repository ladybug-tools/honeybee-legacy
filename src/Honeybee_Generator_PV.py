...
Provided by Honeybee 0.0.56
    ...
    Args:
        _HBSurfaces: A Honeybee surface or a list of Honeybee surfaces to which one Photovolatic generator will be mounted on each surface.
    ...
    Returns:
        PV_HBSurfaces: The Honeybee surfaces that have been modified by this component - these surfaces now contain PV generators to run in an Energy Plus simulation connect them to the HBContext_ input of the Honeybee_ Run Energy Simulation component to run in an EnergyPlus simulation.
...
def checktheinputs(_name,_HBSurfaces,SA_solarcells,cells_n,_integrationmode,No_parallel,No_series):
    ...
    if (_HBSurfaces == []) or (_HBSurfaces[0] == None) :
        print "PV generators must be mounted on at least one Honeybee surface please connect a Honeybee surface to _HBSurfaces!"
        w = gh.GH_RuntimeMessageLevel.Warning
        ghenv.Component.AddRuntimeMessage(w, "PV generators must be mounted on at least one Honeybee surface please connect a Honeybee surface to _HBSurfaces!")
        return -1
